...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6432_Series_ColorSpaceLoadYuv2RgbTable.c
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
#if(_COLOR_CONVERSION_TYPE == _COLOR_CONVERSION_GEN_2)
BYTE code tCOLOR_CONV_YUV2RGB_ITU601_254LIMIT[] =
{
    25, _AUTOINC, (BYTE)P14_A2_SR_YCC2RGB_COEF_K11_MSB,
    0x64, 0x4C, 0x26, 0x67, 0x41, 0x92,
    0xE3, 0x42, 0xC8, 0x17, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x02, 0x00,
    0x02, 0x00, 0x02, 0x00,
    _END
...
BYTE code tCOLOR_CONV_YUV2RGB_ITU709_254LIMIT[] =
{
    25, _AUTOINC, (BYTE)P14_A2_SR_YCC2RGB_COEF_K11_MSB,
    0x64, 0x4C, 0x27, 0x31, 0x00, 0xDB,
    0x42, 0x23, 0x88, 0x79, 0x00, 0x10,
    0x00, 0x10, 0x00, 0x10, 0x02, 0x00,
    0x02, 0x00, 0x02, 0x00,
    _END
...
BYTE code tCOLOR_CONV_YUV2RGB_ITU709_RGBLIMIT_254LIMIT[] =
{
    25, _AUTOINC, (BYTE)P14_A2_SR_YCC2RGB_COEF_K11_MSB,
    0xE4, 0xAB, 0x27, 0x31, 0x00, 0xDB,
    0x42, 0x23, 0x88, 0x79, 0xF0, 0x00,
    0xF0, 0x00, 0xF0, 0x00, 0x02, 0x26,
    0x02, 0x26, 0x02, 0x26,
    _END
};

// YCC to RGB Table - 8bit
BYTE code tCOLOR_CONV_YUV2RGB_ITU601_254LIMIT_8BIT[] =
{
    25, _AUTOINC, (BYTE)P14_A2_SR_YCC2RGB_COEF_K11_MSB,
    0x24, 0x49, 0x66, 0x62, 0x21, 0x91,
    0x63, 0x40, 0xA8, 0x11, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x02, 0x00,
    0x02, 0x00, 0x02, 0x00,
    _END
...
BYTE code tCOLOR_CONV_YUV2RGB_ITU709_254LIMIT_8BIT[] =
{
    25, _AUTOINC, (BYTE)P14_A2_SR_YCC2RGB_COEF_K11_MSB,
    0x24, 0x49, 0xC7, 0x2B, 0x60, 0xDA,
    0xA2, 0x21, 0x28, 0x73, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x02, 0x00,
    0x02, 0x00, 0x02, 0x00,
    _END
...
BYTE code tCOLOR_CONV_YUV2RGB_ITU709_RGBLIMIT_254LIMIT_8BIT[] =
{
    25, _AUTOINC, (BYTE)P14_A2_SR_YCC2RGB_COEF_K11_MSB,
    0x64, 0xA8, 0xC7, 0x2B, 0x60, 0xDA,
    0xA2, 0x21, 0x28, 0x73, 0xF0, 0x00,
    0xF0, 0x00, 0xF0, 0x00, 0x82, 0x24,
    0x82, 0x24, 0x82, 0x24,
    _END
};
#endif
...
void ScalerColorSpaceLoadYuv2RgbTable(EnumColorSpace enumColorSpace, bit bUltraVividSupport)
{
       ...
       (GET_COLOR_RGB_LIMIT_RANGE_USER() == _TRUE))
    {
        ...
        {
#if(_COLOR_CONVERSION_TYPE == _COLOR_CONVERSION_GEN_2)
            if(GET_COLOR_RGB_254LIMIT_RANGE_USER() == _TRUE)
            {
                pucYUV2RGBTable = tCOLOR_CONV_YUV2RGB_ITU709_RGBLIMIT_254LIMIT;
            }
            else
#endif
            {
        ...
        else
        {
            ...
            if(GET_COLOR_RGB_254LIMIT_RANGE_USER() == _TRUE)
            {
                pucYUV2RGBTable = tCOLOR_CONV_YUV2RGB_ITU709_RGBLIMIT_254LIMIT_8BIT;
            }
            else
#endif
            {
    ...
    else
    {
        ...
        switch(GET_COLORIMETRY())
        {
            ...
            case _COLORIMETRY_YCC_XVYCC601:
                ...
                {
#if(_COLOR_CONVERSION_TYPE == _COLOR_CONVERSION_GEN_2)
                    if(GET_COLOR_YCC_254LIMIT_RANGE_USER() == _TRUE)
                    {
                        pucYUV2RGBTable = tCOLOR_CONV_YUV2RGB_ITU601_254LIMIT;
                    }
                    else
#endif
                    {
                ...
                else
                {
                    ...
                    if(GET_COLOR_YCC_254LIMIT_RANGE_USER() == _TRUE)
                    {
                        pucYUV2RGBTable = tCOLOR_CONV_YUV2RGB_ITU601_254LIMIT_8BIT;
                    }
                    else
#endif
                    {
            ...
            case _COLORIMETRY_YCC_XVYCC709:
                if(GET_COLOR_DEPTH() > _COLOR_DEPTH_8_BITS)
                {
                    ...
                    if(GET_COLOR_YCC_254LIMIT_RANGE_USER() == _TRUE)
                    {
                        pucYUV2RGBTable = tCOLOR_CONV_YUV2RGB_ITU709_254LIMIT;
                    }
                    else
#endif
                    {
                ...
                else
                {
                    ...
                    if(GET_COLOR_YCC_254LIMIT_RANGE_USER() == _TRUE)
                    {
                        pucYUV2RGBTable = tCOLOR_CONV_YUV2RGB_ITU709_254LIMIT_8BIT;
                    }
                    else
#endif
                    {
