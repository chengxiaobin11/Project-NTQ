...
void ScalerTimer1SetTimerCount(WORD usTimerMs)
{
    ...
    else
    {
        if(usTimerMs <= _TIMER01_STEP_TIME_MS)
        {
            ...
            // An intermediate variable
#if(_IR_VIRTUAL)    
                PDATA_WORD(0) = usTimerMs*100;
                g_usTimerEventPeriod1 = 1;
            
#elif(_LED_EFFECT_TJMODE14==_ON)
                PDATA_WORD(0) = usTimerMs * 100;  //100
                g_usTimerEventPeriod1 = 10;
#else
                // Record the timer event period is going to happen
                g_usTimerEventPeriod1 = 1;
...
{
    BYTE ucEventIndex = 0;
	//DebugMessageSystem("usTime1111111111111111111-------", usTime);
    ...
    for(ucEventIndex = 0; ucEventIndex < _TIMER_EVENT_COUNT; ucEventIndex++)
    {
        if(GET_EVENT_VALID(ucEventIndex) == _FALSE)
        {
			...
			//DebugMessageSystem("usTime2222222222222222222222-------", usTime);
			//DebugMessageSystem("ScalerTimerCheckTimerEvent(usTime) -------", ScalerTimerCheckTimerEvent(usTime));
            return;
        }
    }
