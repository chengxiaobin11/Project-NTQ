...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_Memory.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
void ScalerMemorySetInputFormat(void);
#if(_FRC_SUPPORT == _ON)
...
void ScalerMemorySetFifoCapture(WORD usHWidth, WORD usVHeight)
{
    ...
    // Set Capture/ Display Line Pixel Number
    ...
    // Set Capture/ Display Line Number
...
WORD ScalerMemoryGetFifoWidth(void)
{
    ...
    //-------------------------------------------------
    pData[2] = ScalerGetDataPortSingleByte(P0_30_FIFO_WINDOW_ADDR_PORT, _P0_31_PT_00_DISPLAY_READ_WID_BSU_H);
    pData[3] = ScalerGetDataPortSingleByte(P0_30_FIFO_WINDOW_ADDR_PORT, _P0_31_PT_01_DISPLAY_READ_WID_BSU_L);
    pData[4] = ScalerGetDataPortSingleByte(P0_30_FIFO_WINDOW_ADDR_PORT, _P0_31_PT_03_DISPLAY_READ_WID_LEN_BSU_H);
...
void ScalerMemorySetInputFormat(void)
{
    ...
    {
        ScalerSetBit(P5_AF_DISP_M1_DISP_CTRL3, ~(_BIT7 | _BIT6), (_BIT7 | _BIT6));
    }
    else
    {
        ScalerSetBit(P5_AF_DISP_M1_DISP_CTRL3, ~(_BIT7 | _BIT6), (_BIT7));
    }
}
...
{
    // Background status
    if(ScalerGetBit(P0_28_VIDEO_DISPLAY_CONTROL_REG, _BIT5) != 0)
    {
        return _TRUE;
    }
...
void ScalerMemoryCalculateLineStepAndBlockStep(DWORD ulHorSize, WORD usVerSize, StructMemoryLineBlockStep *pstMemoryLineBlockStep)
{
    ...
    pstMemoryLineBlockStep->ulLineStep += (pstMemoryLineBlockStep->ulLineStep & 0x01);
