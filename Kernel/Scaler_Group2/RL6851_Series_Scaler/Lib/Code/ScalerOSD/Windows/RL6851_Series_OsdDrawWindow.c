...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_OsdDrawWindow.c
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
void ScalerOsdDrawWindow(WORD usXStart, WORD usYStart, WORD usXEnd, WORD usYEnd)
{
    ...
    if((GET_OSD_WINDOW_GRADIENT_ENABLE() == _DISABLE) && (GET_OSD_WINDOW_ROTATION_FUNCTION_ENABLE() == _ENABLE))
    {
        // Avoid HW bug : window color cannot be change.
