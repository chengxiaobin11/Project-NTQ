...
#define _ADOBE_SRGB_PRECSISON       (_OCC_GAMUT_NO_COMPRESSION_SUPPORT ? _SRGB_1_BIT_SHIFT_LEFT : _SRGB_0_BIT_SHIFT_LEFT)
...
//--------------------------------------------------
#define _DISP_CALIB_FW_VERSION      0x0016
...
void UserCommonDdcciDispCalibSetOCCPCMGamma(void);
#endif
...
void UserCommonDdcciDispCalibProcess(void)
{
    ...
    BYTE ucFlag = 1;
#if((_OGC_SUPPORT == _ON) || (_UNIFORMITY_SUPPORT == _ON) || (_FREESYNC_II_SUPPORT == _ON) || (_OCC_PCM_GAMMA_SUPPORT == _ON))
    BYTE ucGammaIndex = 0;
#endif
...
//****************************************************************************************//
        ...
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_CHANGE_PATTERN)
        {
            if((g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_LUT_LENGTH] == 2) && (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_LUT_ID] == 0xFF))
            {
                usR = (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_RCODE] << 8) | (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_RCODE + 1]);
                usG = (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GCODE] << 8) | (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GCODE + 1]);
                usB = (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_BCODE] << 8) | (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_BCODE + 1]);
                UserCommonDdcciDispCalibShowCalibPattern(usR, usG, usB);
                g_ucPatternIndex += 1;
            }
            else
            {
                if(g_ucPatternIndex == 1)
                {
                    /****************** Save RGB Gain to Flash******************/
                    /* CT/Gamma   Gamma1     Gamma2    Gamma3   Gamma4   Gamma5*/
                    /*    CT1     R/G/B      R/G/B     R/G/B    R/G/B    R/G/B */
...
//****************************************************************************************//
        ...
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_SET_PCM)
        {
            ...
            {
                ScalerColorPCMSRGBAdjust(_ADOBE_SRGB_PRECSISON, g_pucDISP_CALIBRATION_FLASH + _OCC_ADOBEMATRIX_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
            }
            ...
            {
                ScalerColorPCMSRGBAdjust(_ADOBE_SRGB_PRECSISON, g_pucDISP_CALIBRATION_FLASH + _OCC_ADOBEMATRIX_ADDRESS + (g_ucBacklightFlag * _OCC_COLOR_MATRIX_TOTAL_SPACE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
            }
            else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_PCM_COLOR_DOMAIN] == _PCM_USER_DEFINE)
            {
        ...
        {
#if(_OGC_TOTAL_DICOM > 1)
            ucDICOMIndex = g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_INDEX];
#else
        ...
        {
            ucGammaIndex = g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GAMMA_INDEX];
            // Set PCM Input Gamma
            ScalerTimerWaitForEvent(_EVENT_DEN_STOP);
            UserCommonDdcciDispCalibSetOCCPCMGamma();
...
#if(_OCC_GAMUT_NO_COMPRESSION_SUPPORT == _ON)
    g_ucOCCMatrixType = (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_TYPE_II] & _BIT5) >> 5;
    if(!g_ucOCCMatrixType)
    {
        ucFlag = 0;
    }
#endif
...
BYTE UserCommonDdcciDispCalibOCCProcess(BYTE ucStep)
{
    ...
    else if(ucStep == ucPatternEnd)
    {
        ...
        if(g_ucBacklightFlag == _MAX_BACKLIGHT_CAL)
        {
            ...
            {
                ScalerColorPCMSRGBAdjust(_ADOBE_SRGB_PRECSISON, g_pucDISP_CALIBRATION_FLASH + _OCC_ADOBEMATRIX_MAX_BRI_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
            }
            else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_PCM_COLOR_DOMAIN] == _PCM_USER_DEFINE)
            {
                ScalerColorPCMSRGBAdjust(_USER_DEFINE_PRECISION, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE_MAX_BRI_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
...
void UserCommonDdcciDispCalibSaveLUT(BYTE ucTotalLength)
{
    ...
    while(ucPointer < (ucTotalLength - 1))
    {
        ...
        if(ucGammaIndex >= (sizeof(g_ppusGammaCount) / sizeof(g_ppusGammaCount[0])))
        {
            return;
        }
...
#if(_OGC_TOTAL_DICOM > 2)
        else if(ucGammaIndex == _DICOM3_POSITION)
        {
            usStartAddr = tGAMMA_INDEX[_DICOM3_POSITION] + (ucRGB) * 2052;
        }
#if(_OGC_TOTAL_DICOM > 3)
        else if(ucGammaIndex == _DICOM4_POSITION)
        {
            usStartAddr = tGAMMA_INDEX[_DICOM4_POSITION] + (ucRGB) * 2052;
        }
#endif
#endif
#endif
...
BYTE UserCommonDdcciDispCalibCalCheckSum(BYTE ucCheckType)
{
    ...
    switch(ucCheckType)
    {
        case _OGC_TYPE:
            ...
            for(usJ = 0; usJ < 3; usJ++)
            {
                for(usI = 0; usI < 2052; usI = usI + usStep)
                {
        ...
        case _DICOM_TYPE:
            ...
            for(usJ = 0; usJ < 3; usJ++)
            {
                ...
                {
                    UserCommonFlashRead(_OGC_FLASH_BANK, ((((WORD)_OGC_FLASH_PAGE) << 12) + _OGC_DICOM3_ADDRESS + usJ * 2052 + usI), 2, pData + 2);
                    pData[0] += pData[2];
                    pData[0] += pData[3];
                }
            }
            ...
            for(usJ = 0; usJ < 3; usJ++)
            {
                for(usI = 0; usI < 2052; usI = usI + 8)
                {
                    UserCommonFlashRead(_OGC_FLASH_BANK, ((((WORD)_OGC_FLASH_PAGE) << 12) + _OGC_DICOM4_ADDRESS + usJ * 2052 + usI), 2, pData + 2);
                    pData[0] += pData[2];
                    pData[0] += pData[3];
                }
            }
...
void UserCommonDdcciDispCalibReplyResult(BYTE ucType)
{
    ...
    switch(ucType)
    {
        ...
        case _DISP_CALIB_GET_FW_ADVANCE:
            ...
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_ADVANCE_1] = ((_OCC_PCM_GAMMA_SUPPORT << 5) | (_OCC_DISPLAY_P3_GAMMA_22_SUPPORT << 4) | (_HDR10_USER_DEFINE_FIXED_POINT_TONEMAPPING_SUPPORT << 3) | (_HDR10_EETF_TONEMAPPING_SUPPORT << 2));
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_ADVANCE_2] = ((_OCC_GAMUT_NO_COMPRESSION_SUPPORT << 3) | (_OGC_NO_INTERPOLATION_SUPPORT << 2));
...
void UserCommonDdcciDispCalibSetOCCPCMGamma(void)
{
    ...
    ScalerColorOCCPCMGammaInputGammaLoadTable(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GAMMA_INDEX]);

#if(_TWO_BRI_CALIBRATION_TYPE != _TWO_BRI_CALIBRATION_OGC_OCC_DICOM_HDR)
    ScalerColorPCMOutputGammaAdjust(g_pucDISP_CALIBRATION_FLASH + _OCC_GAMMA10_ADDRESS, _OCC_GAMMA_FLASH_BANK);
    // Set PCM Color Matrix
    if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE1)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE2)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE2_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE3)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE3_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE4)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE4_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE5)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE5_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else // _PCM_GAMMA_CT_MODE6
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE6_ADDRESS, _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }

#else
    ScalerColorPCMOutputGammaAdjust(g_pucDISP_CALIBRATION_FLASH + _OCC_GAMMA10_ADDRESS + (g_ucBacklightFlag * _OCC_GAMMA_TABLE_SIZE), _OCC_GAMMA_FLASH_BANK);

    // Set PCM Color Matrix
    if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE1)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE_ADDRESS + (g_ucBacklightFlag * _OCC_COLOR_MATRIX_TOTAL_SPACE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE2)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE2_ADDRESS + (g_ucBacklightFlag * _OCC_COLOR_MATRIX_TOTAL_SPACE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE3)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE3_ADDRESS + (g_ucBacklightFlag * _OCC_COLOR_MATRIX_TOTAL_SPACE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE4)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE4_ADDRESS + (g_ucBacklightFlag * _OCC_COLOR_MATRIX_TOTAL_SPACE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] == _OCC_PCM_GAMMA_CT_MODE5)
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE5_ADDRESS + (g_ucBacklightFlag * _OCC_COLOR_MATRIX_TOTAL_SPACE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
    else // _PCM_GAMMA_CT_MODE6
    {
        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_USERDEFINE6_ADDRESS + (g_ucBacklightFlag * _OCC_COLOR_MATRIX_TOTAL_SPACE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
    }
#endif

    ScalerColorOutputGammaEnable(_FUNCTION_ON);
    if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_CT_INDEX] != 0x0F)
    {
        ScalerColorSRGBEnable(_FUNCTION_ON);
    }
    ScalerColorPCMInputGammaEnable(_FUNCTION_ON);
