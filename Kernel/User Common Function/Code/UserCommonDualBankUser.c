...
BYTE UserCommonDualBankUserGetVerifyCopyResult(void)
{
    ...
    BYTE ucResult = 0;
    DWORD ulUserFlagAddr = (DWORD)((DWORD)_DUAL_BANK_USER_I_USER_DATA_BANK_ADDR << 16) + _DUAL_BANK_USER_I_FLAG_START_ADDR;

#if((_DUAL_BANK_TYPE == _DUAL_BANK_DIFF_BANK) || (_DUAL_BANK_TYPE == _DUAL_BANK_COPY_BANK))
    DWORD ulUser2FlagAddr = 0;
    BYTE pucUser2Flag[_DUAL_BANK_USER_FLAG_LENGTH] = {0};

    ulUser2FlagAddr = (DWORD)((DWORD)_DUAL_BANK_USER_II_USER_DATA_BANK_ADDR << 16) + _DUAL_BANK_USER_II_FLAG_START_ADDR;
