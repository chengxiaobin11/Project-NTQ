...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_OD.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6851_SERIES_OD__
...
// DEFINITIONS / MACROS
//****************************************************************************
#define _OD_ROW_ADDR_SHIFT_BIT_NUM                  7
...
void ScalerODSetMemoryStartAddress(void)
{
    ...
    // Unit: 64 bit (OD memory access unit)
    ulODUseSpace = CEILING(ulODUseSpace, _OD_FIFO_BUS_WIDTH);

    // Unit: memory row
    ulODUseSpace = ulODUseSpace / _MEMORY_BIT_NUM / (_MEMORY_COL_PER_BANK * _MEMORY_BANK) + 1;

    ulStartAddress = ((DWORD)_MEMORY_ROW - ulODUseSpace) << _OD_ROW_ADDR_SHIFT_BIT_NUM;

    // Set Memory Start Address
    ScalerSetBit(P3_DA_LS_DDR_START_ADDR_MSB, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (BYTE)((ulStartAddress & 0x0F000000U) >> 24));
    ScalerSetByte(P3_DB_LS_DDR_START_ADDR_H, (BYTE)((ulStartAddress & 0x00FF0000U) >> 16));
    ScalerSetByte(P3_DC_LS_DDR_START_ADDR_M, (BYTE)((ulStartAddress & 0x0000FF00U) >> 8));
    ScalerSetByte(P3_DD_LS_DDR_START_ADDR_L, (BYTE)(ulStartAddress & 0x000000FFU));
}
