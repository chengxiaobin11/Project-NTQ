...
bit ScalerDpTxHdcp2AkeInitial(void)
{
    ...
    SET_HDCP2_2_RANDOM_GENERATOR_DISABLE();
...
bit ScalerDpTxHdcp2ContentStreamManagement(void)
{
    ...
    if(GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_MST)
    {
        ...
        {
            BYTE ucStreamNumber = ScalerDpRxMstGetSourceMuxBackup(enumStSourceTemp);

            if(ucStreamNumber != _DP_ST_NONE)
            {
                // Get Stream Source X Status
                ucStreamId = ucStreamNumber;
    ...
    else if(GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_MST2SST)
    {
        ...
        if(ScalerSyncDpGetHdcp2ModeStatus() == _TRUE)
        {
            ...
            // Get Stream Source X Status
            ucStreamId = ScalerDpRxMstGetSourceMuxBackup(_DP_ST_SOURCE2);
    ...
    if((ScalerSyncDpHdcp2GetContentStreamManagementStatus() == _TRUE) && (GET_DP_TX_HDCP2_CONTENT_STREAM_MANAGEMENT() == _FALSE))
    {
                ...
                (GET_DP_TX_HDCP_AUTH_STATE() >= _DP_TX_HDCP_STATE_AUTH_2))
        {
            ...
            if(GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_MST)
            {
                ...
                for(enumStSourceTemp = _DP_ST_SOURCE3; enumStSourceTemp <= _DP_ST_SOURCE5; enumStSourceTemp++)
                {
                    if(ScalerDpRxMstGetSourceMuxBackup(enumStSourceTemp) != _DP_ST_NONE)
                    {
                        // Get Stream Source X Status
                        ucStreamId = ScalerDpRxMstGetSourceMuxBackup(enumStSourceTemp);
