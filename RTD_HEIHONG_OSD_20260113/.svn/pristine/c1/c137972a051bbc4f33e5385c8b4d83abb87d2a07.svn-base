...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_ScalerADC.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
void ScalerADCThermalCompensationSetting(void)
{
    // Set ADC S&H thermal compensation.
    ScalerSetBit(P0_DA_ADC_REG2, ~(_BIT3 | _BIT2 | _BIT1), 0x00); // default, 50ps

    // Set ADC clock (Red) thermal compensation.
    ScalerSetBit(P0_CF_ADC_CTL_RED, ~(_BIT2 | _BIT1 | _BIT0), 0x00);
    // Set ADC clock (Grm) thermal compensation.
    ScalerSetBit(P0_D0_ADC_CTL_GRN, ~(_BIT2 | _BIT1 | _BIT0), 0x00);
    // Set ADC clock (Blu) thermal compensation.
    ScalerSetBit(P0_D1_ADC_CTL_BLU, ~(_BIT2 | _BIT1 | _BIT0), 0x00);

    // Set APLL phase0 thermal compensation.
    // {0x01AB[4],0x01FF[7:6]} = 3'b011, default 1.126ns
    ScalerSetBit(P1_AB_APLL_RESERVED2, ~(_BIT4), 0x00);
    ScalerSetBit(P1_FF_APLL_THERMO_CTRL_0, ~(_BIT7 | _BIT6), (_BIT7 | _BIT6));


    // APLL & ADC Path thermal Compensation Setting
    if(ScalerGetBit(P0_47_SYNC_SELECT, _BIT4) == _BIT4) // SOG mode
    {
        // APLL thermal compensation need set by w/i long delay path
