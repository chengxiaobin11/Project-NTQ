...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_ScalerCommonDPTxAux.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
bit ScalerDpTxAuxRead(BYTE ucCommandAddrH, BYTE ucAddrM, BYTE ucAddrL, BYTE ucLength, BYTE *pucReadArray)
{
    ...
    do
    {
        ...
        while((ucReadRetryNum <= _DP_AUX_TX_RETRY_LIMIT) && (ScalerGetBit(PBD_AE_AUX_FIFO_WR_PTR, (_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0)) == 0x00));
    }
    while((ucCheckLength < ucLength) && (ucReadRetryNum <= _DP_AUX_TX_RETRY_LIMIT));
...
BYTE ScalerDpTxAuxNativeRead_EXINT0(BYTE ucAddrH, BYTE ucAddrM, BYTE ucAddrL, BYTE ucLength, BYTE *pucReadArray) using 1
{
    ...
    do
    {
        ...
        while((ucReadRetryNum <= _DP_AUX_TX_RETRY_LIMIT) && (ScalerGetBit_EXINT(PBD_AE_AUX_FIFO_WR_PTR, (_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0)) == 0x00));
    }
    while((ucCheckLength < ucLength) && (ucReadRetryNum <= _DP_AUX_TX_RETRY_LIMIT));
