...
#define _CONTRAST_VGA                               _DEF_CONTRAST
#define _CONTRAST_DVI                               _DEF_CONTRAST
#define _CONTRAST_VIDEO8                            _DEF_CONTRAST
#define _CONTRAST_HDMI                              _DEF_CONTRAST
#define _CONTRAST_DP                                _DEF_CONTRAST
#else
#define _CONTRAST_VGA                               _DEF_CONTRAST
#define _CONTRAST_DVI                               _DEF_CONTRAST
#define _CONTRAST_VIDEO8                            _DEF_CONTRAST
...
#define _BRIGHTNESS_VGA                             _DEF_BRIGHTNESS
#define _BRIGHTNESS_DVI                             _DEF_BRIGHTNESS
#define _BRIGHTNESS_VIDEO8                          _DEF_BRIGHTNESS
#define _BRIGHTNESS_HDMI                            _DEF_BRIGHTNESS
#define _BRIGHTNESS_DP                              _DEF_BRIGHTNESS
#else
#define _BRIGHTNESS_VGA                             _DEF_BRIGHTNESS
#define _BRIGHTNESS_DVI                             _DEF_BRIGHTNESS
#define _BRIGHTNESS_VIDEO8                          _DEF_BRIGHTNESS
...
code char OsdDataFlag[] = {"This is OsdDefaultData Flag!"};
...
//91+12 = 103
#if(_DEF_PAGE8)
BYTE code OsdStateDefineDefault[_OSD_MAIN_MENU_COUNT][7]={     
...
#elif(_OSD_7PAGE)
BYTE code OsdStateDefineDefault[_OSD_MAIN_MENU_COUNT][6]={     
...
code StructFlag g_stFlag =			
{
	_NAVIGAT_MENU_SURPPORT,  //b1NavigatMenuFlag	
	_CTS_TYPE, 				 //b1CTS_TYPE
	_ADJ_BACKLIGHT_BY_TABLE, //b1BACKLIGHT_BY_TABLE
...
code StructOsdUserDataType g_stOSDDefaultData =  //+29
{
    _DEF_OSD_BACKLIGHT,       // ucBackLight;
    _DEF_H_POS,               // ucOsdHPos;
    _DEF_V_POS,               // ucOsdVPos;

    _DEF_TIMEOUT,                       // ucOsdTimeout;
    0,                        // ucAspectOriginRatio

    32,                        // ucTransparency;
    
	_DEF_SAVING_MODE,           //ucOsdSavingMode;
//--------------------------------------------------------
    _DEF_LANGUAGE,            // b4Language;
    _DEF_ECO,   			  // b4ColorEffect;
//--------------------------------------------------------
    _CT_DEFAULT,            // b4ColorTempType;
    _COLOR_SPACE_RGB,         // b3VGARGBYUV;
    _COLOR_SPACE_RGB,         // b3DVIRGBYUV;
//--------------------------------------------------------
    _DEF_SHAPNESS,            // b3Sharpness;
    _DEF_OSDROTATE,           // b2OsdRotate;
    _OSD_ROTATE_DISPLAY_KEEP_SRC_ASPECT_RATIO,     // b2DispRotationSizeType;
    _OFF,                      // b1OverScan;
//--------------------------------------------------------
	_DEF_GAMMA,               // b3Gamma;
	_DEF_ASPECT,			  // b3AspectRatio;
	_ON,                      // b1DDCCIStatus;
    _DEF_DCR,                 // b1OsdDcrStatus ;
//--------------------------------------------------------
    _OD_GAIN_DEF,          // ucODGain;
    _DEFAULT_HUE,             // cHue;
...
//--------------------------------------------------------
    ...
    _DEF_AUDIO_SOURCE,        // b1AudioSourceStatus : 1;
//--------------------------------------------------------
    _DEF_VOLUME,               // ucVolume;
//--------------------------------------------------------
    _DEF_OD,                   // b1ODStatus : 1;
...
#endif
//--------------------------------------------------------
   _DP_VER_1_DOT_2,// _DP_VER_1_DOT_3,                         // b1OsdDpD0PortVersion : 1;//_DP_1_DOT_1
   _DP_VER_1_DOT_2,//_DP_VER_1_DOT_3,                         // b1OsdDpD1PortVersion : 1;
    _OFF,                      // b2OsdDpMST : 2;
    _DEF_DISP_ROTATE,          // b3OsdDispRotate : 3;
...
//--------------------------------------------------------
    ...
    _DEF_FREESYNC,             // b1FreeSyncStatus : 1;
    _OSD_CLONE_OFF,            // b1CloneMode: 1;
//--------------------------------------------------------
    _LATENCY_L,                // b2LatencyStatus : 2
    _DEF_OSD_DOUBLE,            // BYTE b1OsdDoubleSize :1;
    _DEF_HDR_MODE,              // BYTE ucHdrMode;
...
// --------------------------------------------------------
	...
	_DEF_OSD_LEDTYPE,	       // BYTE b2OSDLedType : 2
	0,						   // b2CrossHairMode1 : 3;
	0,						   // b3CrossHairMode2 : 3;

	0,	   			   		   // BYTE ucLowBlue

	_DEF_POWEROFF,//_AutoPowerOff_4,	//ucOSDAutoPowerOff
	_DEF_HOTKEY_LEFT-_MENU_FUN_BACKLIGHT,          // BYTE ucHotKeyLeft
	_DEF_HOTKEY_RIGHT-_MENU_FUN_BACKLIGHT,         // BYTE ucHotKeyRight
	_DEF_FLICKER_FREE,						// b1FlickerFree
	_DEF_MPRT,//b1MPRT
	_OFF,  //b1OverClock
	...
	0,//b1Burning_Audio_Test
	0,//	b1Align	

	
	1,//b1EnergeNoticeflag  �ܺ����ѱ�־λ
	0,//b1OsdLock  �˵�����־λ
	0,//b1PowerkerLock
	_OFF,//b1FvInformation
	_COLOREFFECT_STANDARD,//b3DcrOffEco
	0,//b1xzHZ
	_OFF,// b1OverClockHint	//_MENU_FUN_CHAOPING_TISHI
	_OFF,//b2Kvm   //_MENU_FUN_KVM  0���Զ�
#if(_DEF_OPPOSITE_DCR)
	_OFF,	//b2OsdDcrOpposite	//_MENU_FUN_DCR_OPPOSITE
#endif
	0,//b3MemoryData
#if(_DEF_HOTKEY_REPLACE)
	/* 0.��  1.����  2.�Աȶ�  3.�龰ģʽ  4.ɫ��  5.��׼��  6.֡��׷��  7.����  8ɫ��  9.������ǿ  10.��̬����ģʽ  11.�ź�����	*/
	5,// b4HotKeyReplaceLeft; //_MENU_FUN_HOT_REPLACE_LEFT
	6,// b4HotKeyReplaceRight	//_MENU_FUN_HOT_REPLACE_RIGHT
	11,//	b4HotKeyReplaceExit //_MENU_FUN_HOT_REPLACE_EXIT
#endif



#if(_DEF_COLORFUL)
	60,//usContrastColorful
#endif

#if(_DEF_STA_SIX_COLOR)
	0,	// b3StaState   _MENU_FUN_SAT_STATE
	0,	// b3HueState   _MENU_FUN_HUE_STATE
#endif
	0,//byHzzhuangtai

#if(_LED_EFFECT_TJMODE14)
_DEF_LIGHTMODE,/*	b1LightMode*/
LightColorMode_Spectrum,/*b1Color*/
_DEF_PreLIGHTMODE ,/*	b1PreLightMode*/
LightColorMode_Spectrum,/*b1PreColor*/
#endif
    0,
}; //35
...
void RTDFlashStartup(void)
{
    ...
    RTDFlashInitialAddr();
	memcpy(OsdStateDefine,OsdStateDefineDefault,sizeof(OsdStateDefineDefault));
...
#endif
#if( (_WRITE_HDCP_TO_SYSTEM_EEPROM == _ON&&_HDCP_KEY_WIRTE_FLAG ==_OFF)||(Project_ID == ID_TJZG_HK2775E05_M270KCAD8B_EDP4LANHBR2_MD2769_QHD180HZ_20240313)||\
...
void RTDFlashLoadBriCon(BYTE ucSource)
{
    if(UserCommonFlashLoadItem(_FLASH_ITEMID_BRICON_SRC0_VGA, pData, sizeof(StructBriConDataType)) == _TRUE)
    {
...
{
	ucSource = ucSource;
    UserCommonFlashSaveItem(_FLASH_ITEMID_BRICON_SRC0_VGA, (BYTE *)(&g_stBriConData), sizeof(StructBriConDataType));
}
...
void RTDFlashRestoreBriCon(void)
{
    ...
    for(ucSourceIndex = 0; ucSourceIndex < 1; ucSourceIndex++)
    {
