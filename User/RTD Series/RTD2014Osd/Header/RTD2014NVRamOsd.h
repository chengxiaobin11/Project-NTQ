...
//--------------------------------------------------------
...
#endif
	BYTE b1OverClock : 1;
	BYTE b1ShowLogoFlag:1;

#if 0//(_DCC_PROGRESS_BAR)
	BYTE blDccFlag:5;
#else
	BYTE blDccFlag:1;
#endif
	...
	BYTE b1Enger :1;
#endif
	BYTE b1Burning_Audio_Test: 1;	


	BYTE b1Align: 1;

	BYTE b1EnergeNoticeflag:1;	//�ܺ����ѱ�־λ
	
	BYTE b1OsdLock : 1;			//�˵�����־λ
	BYTE b1PowerkerLock : 1;	//��Դ����־λ

	BYTE b1FvInformation:1;		//��ʾƵ�ʱ�־λ
	BYTE b3DcrOffEco :3;		//DCR�رպ�ԭECOģʽ	
	BYTE b1xzHZ :1;
	BYTE b1OverClockHint:1;	//_MENU_FUN_CHAOPING_TISHI
	BYTE b2Kvm:2;	//_MENU_FUN_KVM
#if(_DEF_OPPOSITE_DCR)
	BYTE b2OsdDcrOpposite:2;//_MENU_FUN_DCR_OPPOSITE
#endif
	BYTE b3MemoryData:3;	//��Ϊһ�����õļ�����ֵ

#if(_DEF_HOTKEY_REPLACE)
	BYTE b4HotKeyReplaceLeft:4;	//_MENU_FUN_HOT_REPLACE_LEFT
	BYTE b4HotKeyReplaceRight:4; //_MENU_FUN_HOT_REPLACE_RIGHT
	BYTE b4HotKeyReplaceExit:4; //_MENU_FUN_HOT_REPLACE_EXIT
#endif

	
#if(_DEF_COLORFUL)
	BYTE usContrastColorful;	//����ģʽ�Աȶ�
#endif

#if(_DEF_STA_SIX_COLOR)
    BYTE b3StaState:3;		//_MENU_FUN_SAT_STATE    
    BYTE b3HueState:3;		//_MENU_FUN_HUE_STATE    
#endif

	BYTE byHzzhuangtai;			//��ʾƵ����֤λ

    
#if(_LED_EFFECT_TJMODE14)
        BYTE b1LightMode : 3;
        BYTE b1Color : 4;
        BYTE b1PreLightMode : 2;
        BYTE b1PreColor : 4;
...
typedef struct
{
	BYTE b1NavigatMenuFlag    :1;
	BYTE b1CTS_TYPE 	      :1;
	BYTE b1BACKLIGHT_BY_TABLE :1;
	BYTE b5INPUT_PORT_VALID   :5;
	BYTE ucA0_INPUT_PORT_TYPE;	
	BYTE ucD0_INPUT_PORT_TYPE;	
	BYTE ucD1_INPUT_PORT_TYPE;
	BYTE ucD2_INPUT_PORT_TYPE;
	BYTE ucD3_INPUT_PORT_TYPE;
	BYTE ucD4_INPUT_PORT_TYPE;
